#pragma once

#include <iostream>
#include <vector>
#include <fstream>
#include <cstdio>
#include <string>
#include <sstream>

#include "miniaudio.h"


struct audioParams
{
    std::vector<unsigned char> audiofile;
    std::string audiofileName, audioDuration, audioAuthor;
    ma_uint64 totalFrames;
    ma_uint64 currentFrame = 0;
    bool loaded;
    float volume;
    bool isPlaying;
};

extern char videoUrl[256];
extern audioParams audio;

extern std::string ytExeDir;
extern std::string ffExeDir;

extern audioParams downloadAudio(const char* url);
extern bool convertAudio(audioParams &audio);

extern bool initAudio(audioParams &audio, ma_decoder &decoder);
extern void unInitAudio(ma_decoder &decoder ,ma_device &device);
extern bool setUpAudioDevice(ma_decoder &decoder ,ma_device &device, audioParams &audio);
extern void playAudio(ma_device &device);
extern void pauseAudio(ma_device &device);
extern void startAudio(ma_device &device);
extern void setAudioVolume(ma_device &device, float volume);